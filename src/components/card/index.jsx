  ...
  return (
    <Link to={`/watch?v=${video.videoId}`} className="group block">
      ...
      <div className="relative rounded-xl overflow-hidden mb-3 w-full aspect-video">
        ...
        {video.lengthText && (
          <span className="absolute bottom-1 right-1 bg-black/80 text-xs px-2 py-0.5 rounded"></span>
        )}
        {video.isLive && (
          <span className="absolute bottom-1 right-1 bg-red-600 text-white  text-xs px-1 py-0.5 rounded">
            CANLI
          </span>
        )}
      </div>
